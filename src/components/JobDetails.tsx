...
import { useLikedJobs, checkIsLiked } from "@/hooks/use-liked-jobs";
...
export default function JobDetails({ jobId }: Props) {
  const { data, isLoading, error } = useSWR<Job[]>(jobId ? jobId : null, getJobDetails);
  console.log(data);
  const job = data && data.length > 0 && data[0];
  const { likedJobs, addLikedJob, removeLikedJob } = useLikedJobs();
  const isLiked = job && checkIsLiked(likedJobs, job.job_id);

  if (isLoading) return <h3>Loading...</h3>;
  if (error) return <h3>Error loading jobs</h3>;

  return (
    <div className="bg-green shadow-md rounded-lg">
      {job && (
